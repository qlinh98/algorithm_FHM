...
import ca.pfv.spmf.test.tools.MemoryLogger;
...
public class AlgoFHM {
	...
	public void runAlgorithm(String input, String output, int minUtility) throws IOException {
		...
		// System.out.println(" ram: " + MemoryLogger.getInstance().checkMemory() + " mb
		// ");

		// Mine the database recursively
		// startTimestamp = System.currentTimeMillis();
		endTimestamp = System.currentTimeMillis();
		System.out.println(" Total time truoc fhm " + (endTimestamp - startTimestamp) + " ms");
		fhm(itemsetBuffer, 0, null, listOfUtilityLists, minUtility);
		endTimestamp = System.currentTimeMillis();
		System.out.println(" Total time sau fhm " + (endTimestamp - startTimestamp) + " ms");
		// check the memory usage again and close the file.
		// System.out.println("ram:" + MemoryLogger.getInstance().checkMemory() + "");
		// close output file
		writer.close();
		// record end time
		endTimestamp = System.currentTimeMillis();
	...
	private void fhm(int[] prefix, int prefixLength, UtilityList pUL, List<UtilityList> ULs, int minUtility)
		...
		for (int i = 0; i < ULs.size(); i++) {
			...
			if (X.sumIutils + X.sumRutils >= minUtility) {
				...
				// System.out.println(itemsetBuffer);
